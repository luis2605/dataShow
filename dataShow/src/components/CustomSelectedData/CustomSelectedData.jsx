...
const CustomSelectedData = ({
  onMultipleFilterData,
  onCustomSelectedData,
  onUserName,
  onAmountProjects,
}) => {
  ...
  const handleSelectionExport = () => {
    ...
    const modifiedArray = onCustomSelectedData.map((item) => {
      ...
      if (
        item.hasVideoOnSocialbnb &&
        item.hasVideoOnSocialbnb.includes('video":"ja')
      ) {
    ...
    XLSX.utils.sheet_add_json(ws, userNameRowData, {
      origin: { r: convertedData.length + 2, c: 1 },
    }); // Start at B2 (0-indexed)
  ...
  const handleTableExport = () => {
    ...
    const updatedData = data.map((row) => {
      ...
      nestedKeysToFormat.forEach((key) => {
        if (formattedRow[key]) {
          ...
          formattedRow[key] = nestedKeys
            .map((nestedKey) => `${nestedKey}: ${nestedObject[nestedKey]}`)
            .join(", ");
        }
      });
      ...
      if (formattedRow["Extra Activities"]) {
        const extraActivities =
          formattedRow["Extra Activities"].split(currency);
    ...
    const userNameRowData = [userNameRow]; // Wrap userNameRow in an array
    XLSX.utils.sheet_add_json(ws, userNameRowData, {
      origin: { r: updatedData.length + 2, c: 1 },
    }); // Start at B2 (0-indexed)
    const amounProjectsData = [projectAmountRow]; // Wrap userNameRow in an array
    XLSX.utils.sheet_add_json(ws, amounProjectsData, {
      origin: { r: updatedData.length + 4, c: 1 },
    }); // Start at B2 (0-indexed)
  ...
  return (
    <div className={classes["secondary-btn-container"]}>
      <Button
        ...
        style={{ marginRight: "1em", background: "#1C7881", border: "none" }}
      >
        {t("customSelectedData.ExportTable")}
      </Button>
      <Button
        id="step5"
        ...
        style={{ marginRight: "1em", background: "#1C7881", border: "none" }}
      >
        {t("customSelectedData.ExportRaw")}
      </Button>

      {onCustomSelectedData && onCustomSelectedData.length === 0 ? (
        ""
      ) : (
        <Button
          ...
          style={{ marginRight: "1em", background: "#1C7881", border: "none" }}
        >
          {t("customSelectedData.ExportSelection")}
        </Button>
      )}
    </div>
  );
