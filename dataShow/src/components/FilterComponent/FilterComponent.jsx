  ...
  useEffect(() => {
    if (jsonData) {
      ...
      if (jsonData && jsonUserData) {
        const hostUsers = jsonUserData.filter(
          (entry) => entry.NumOfOpenListings > 0
        );
        setHosts(hostUsers);
        filteredData = jsonData.map((data) => {
          const matchingUser = jsonUserData.find(
          ...
          if (matchingUser) {
            return {
              ...
              hasVideoOnSocialbnb: matchingUser.PrivateData.includes(
                'video":"ja'
              )
                ? "ja"
                : "nein",
      ...
      if (
        ...
        filteredData = filteredData.filter((item) => {
          ...
          const countryMatch =
            !selectedCountry || publicData.country === selectedCountry;
          const roomtypeMatch =
            !selectedRoomtype || publicData.roomtype === selectedRoomtype;
          const bathroomMatch =
            !hasPrivateBathroom ||
            (publicData &&
              publicData.amenities &&
              publicData.amenities.includes("privat_bathroom"))
              ? "yes"
              : "no" == hasPrivateBathroom;
          const bathroomMatchNot =
            !hasPrivateBathroom ||
            (publicData &&
              publicData.amenities &&
              publicData.amenities.includes("shared_bathroom"))
              ? "no"
              : "yes" == hasPrivateBathroom;
          const hasVideoOnSocialbnbMatch =
            !hasVideoOnSocialbnb ||
            (hosts &&
              hosts.length > 0 &&
              item.hasVideoOnSocialbnb == hasVideoOnSocialbnb);

          const cityMatch = !selectedCity || publicData.city === selectedCity;
          const activitiesMatch =
            !offerActivities ||
            (publicData.activities ? "yes" : "no") === offerActivities;
          const impactMatch =
            !impact ||
          ...
          const continent =
            publicData &&
            publicData.country &&
            getContinentName(publicData.country);
      ...
      const mapped = filteredData.map((item, index) => {
        ...
        if (
          item.publicData1 &&
          item.publicData1.food &&
          item.publicData1.food.includes("breakfast_inclusive") &&
          item.publicData1 &&
          item.publicData1.food &&
          !item.publicData1.food.includes("lunch_inclusive") &&
          item.publicData1 &&
          item.publicData1.food &&
          !item.publicData1.food.includes("dinner_inclusive")
        ) {
        ...
        if (
          (item.publicData1 &&
            item.publicData1.amenities &&
            !item.publicData1.amenities.includes("private_kitchen")) ||
          (item.publicData1 &&
            item.publicData1.amenities &&
            !item.publicData1.amenities.includes("shared_kitchen")) ||
          (item.publicData1 &&
            item.publicData1.food &&
            !item.publicData1.food.includes("lunch_for_sale")) ||
          (item.publicData1 &&
            item.publicData1.food &&
            !item.publicData1.food.includes("dinner_for_sale")) ||
          (item.publicData1 &&
            item.publicData1.food &&
            !item.publicData1.food.includes("breakfast_for_sale"))
        ) {
          meals = "N/A";
        }
        ...
        const url = `https://www.socialbnb.org/l/${name.replace(
          /\s+/g,
          "-"
        )}/${id}`;
        ...
        const singleExtraActivity =
          jsonUserData && item.actividades_nuevas
            ? JSON.parse(item.actividades_nuevas)
            : null;
        let activitiesArray = [];

        if (singleExtraActivity) {
          activitiesArray = Object.entries(singleExtraActivity).reduce(
            (acc, [key, value]) => {
        ...
        return isBrowser ? (
          <tr key={index} className={classes.card}>
            ...
            {showExtraCategories && (
              <td>
                {item.publicData1.bedamount && item.publicData1.roomamount
                  ? item.publicData1.bedamount * item.publicData1.roomamount
                  : ""}
              </td>
            )}
            ...
            {item.publicData1 &&
            item.publicData1.amenities &&
            item.publicData1.amenities.includes("privat_bathroom") ? (
            ...
            <td>
              <div className={classes["act-exp-container"]}>
                <span
                  className={
                    actExpanded ? classes["act-exp"] : classes["act-not-exp"]
                  }
                >
                ...
                <div
                  className={
                    ...
                    !activitiesArray.some(
                      (item) => item.key === "K/A" && item.value === "K/A"
                    )
                      ? classes.activitiesDisplayTable
                      : ""
                  }
                >
            ...
            {item.publicData1.category && (
              <td>
                {Array.isArray(item.publicData1.category)
                  ? item.publicData1.category.join(" ")
                  : item.publicData1.category}
              </td>
            )}
            <td>
              {Array.isArray(item.publicData1.languages)
                ? item.publicData1.languages.join(" ")
                : item.publicData1.languages
                ? item.publicData1.languages
                : "N/A"}
            </td>
            ...
            {showExtraCategories && (
              <td>
                <a href={item.publicData1.youtubeLink} target="_blank">
                  <i
                    className="fa-brands fa-youtube"
                    style={
                      item.publicData1.youtubeLink &&
                      item.publicData1.youtubeLink.length > 0
                        ? youtubeLinkButtonStyle
                        : youtubeLinkButtonNA
                    }
      ...
      const countriesArray = Object.entries(countryCounts).map(
      ...
      const removeCustomElement = (e) => {
        ...
        const updatedSelectedCustomData = selectedCustomData.filter(
          (_, index) => index !== parseInt(elementIndex)
      ...
      setPercent(() => {
        return calculatePercent(jsonData.length, filteredData.length).toFixed(
          2
        );
      });
      ...
      if (
        multipleFilterData &&
        jsonData &&
        multipleFilterData.length < jsonData.length
      ) {
    ...
    console.log(multipleFilterData);
    {
      multipleFilterData &&
        multipleFilterData.forEach((item) => {
          ...
          if (
            projectTitle !== prevProjectTitle &&
            projectLocation !== prevProjectLocation
          ) {
        ...
        <div key={"mxps"} className={classes["mega-container"]}>
          ...
          {jsonData && (
            <div
              className={
                dataHasBeenFiltered
                  ? `${classes["utilities-bar"]} ${classes["utilities-bar-single"]}`
                  : classes["utilities-bar"]
              }
            >
              {isBrowser && !dataHasBeenFiltered && (
                <img
                  className={classes["select-filter-img"]}
              ...
              <div className={classes["main-btn-container"]}>
                ...
                <h4 style={{ margin: "1em 0" }}>
                  ...
                  {fileMetadata.lastModifiedDate.toLocaleString()}
                </h4>
                {fileUserMetadata && (
                  <h4 style={{ margin: "1em 0" }}>
                    {" "}
                    {t("usersMetadata.DownloadedOn")}{" "}
                    {fileUserMetadata.lastModifiedDate.toLocaleString()}
                  </h4>
                )}
                <h4 style={{ margin: "1em 0" }}>
                  {" "}
                  {t("Metadata.ReportCreatedBy")} {onUserName}
                </h4>
                <CustomSelectedData
                  onMultipleFilterData={multipleFilterData}
                  onCustomSelectedData={selectedCustomData}
                  onUserName={onUserName}
                  onAmountProjects={projectTitleCounts[uniqueProjectTitles[0]]}
                />
                {!selectedCountry && (
                  <Button
                    id="step6"
          ...
          <Offcanvas show={show} onHide={handleClose}>
            ...
            <Offcanvas.Body>
                ...
                <div className={classes["nav-container"]}>
                  <div className={classes["filter-container"]}>
                    ...
                    </div>
                    {
                      ...
                      >
                        <div className={classes["filter-element"]}>
                          <label htmlFor="selectedContinent">
                            {t("filterCanvas.Continents")}
                          </label>
                          <select
                          ...
                          >
                            <option value="">
                              {t("filterCanvas.ContinentsOption1")}
                            </option>
                            <option key={1} value={"Africa"}>
                              {t("filterCanvas.ContinentsOption2")}
                            </option>
                            <option key={2} value={"Antarctica"}>
                              {t("filterCanvas.ContinentsOption3")}
                            </option>
                            <option key={3} value={"Asia"}>
                              {t("filterCanvas.ContinentsOption4")}
                            </option>
                            <option key={4} value={"Europe"}>
                              {t("filterCanvas.ContinentsOption5")}
                            </option>
                            <option key={5} value={"North America"}>
                              {t("filterCanvas.ContinentsOption6")}
                            </option>
                            <option key={6} value={"Oceania"}>
                              {t("filterCanvas.ContinentsOption7")}
                            </option>
                            <option key={7} value={"South America"}>
                              {t("filterCanvas.ContinentsOption8")}
                            </option>
                          </select>
                        </div>
                      </OverlayTrigger>
                    ...
                    {jsonUserData && (
                      <div className={classes["filter-element"]}>
                        <label htmlFor="hasVideoOnSocialbnb">
                          socialbnb Video
                        </label>
                        <select
                          ...
                          onChange={(e) =>
                            setHasVideoOnSocialbnb(e.target.value)
                          }
                        >
                          <option value="">
                            {t("filterCanvas.BathroomOption1")}
                          </option>
                          <option key={1} value={"ja"}>
                            yes
                          </option>

                          <option key={2} value={"nein"}>
                            no
                          </option>
                        </select>
                      </div>
                    ...
                    <div className={classes["filter-element"]}>
                      ...
                      >
                        ...
                        {[
                          ...new Set(
                            multipleFilterData.map(
                              (item) => item.publicData1.country
                            )
                          ),
                        ].map((country, index) => (
                    ...
                    <div className={classes["filter-element"]}>
                      ...
                      >
                        <option value="">
                          {t("filterCanvas.RoomTypeOption1")}
                        </option>
                    ...
                    <div className={classes["filter-element"]}>
                      <label htmlFor="bathroomFilter">
                        {t("filterCanvas.Bathroom")}
                      </label>
                      <select
                        id="bathroomFilter"
                    ...
                    <div className={classes["filter-element"]}>
                      <label htmlFor="activitiesFilter">
                        {t("filterCanvas.Activities")}
                      </label>
                      <select
                        id="activitiesFilter"
                      ...
                      >
                        <option value="">
                          {t("filterCanvas.ActivitiesOption1")}
                        </option>
                        {/* the values are not available on json data*/}
                  ...
                  <div className={classes["btn-container"]}>
                    ...
                    <div>
                      <button
                        ...
                        onClick={resetQueryText}
                      >
                        <i
                          className="fa  fa-delete-left"
                          style={{ fontSize: "16px" }}
                        ></i>
                      </button>
                      <button
              ...
              <div
                className={classes["custom-shape-divider-bottom-1685701521"]}
              >
                <svg
                  data-name="Layer 1"
                  ...
                  <Table striped bordered hover id="filteredTable">
                    <thead>
                      <tr>
                        ...
                        {showExtraCategories && (
                          <th>{t("table.RoomAmount")}</th>
                        )}
                        {showExtraCategories && (
                          <th>{t("table.RoomOccupancy")}</th>
                        )}
                        {showExtraCategories && <th>{t("table.MaxAmount")}</th>}
                        {showExtraCategories && (
                          <th>{t("table.Price/Night")}</th>
                        )}
