...
const LanguageSelector = ({ onJsonData }) => {
  ...
  const currentLangClasses = onJsonData
    ? classes["current-lang-blue"]
    : classes["current-lang"];
  const langTextClasses = onJsonData
    ? classes["lang-text-white"]
    : classes["lang-text"];
  const langDropdownClasses = onJsonData
    ? classes["lang-dropdown"]
    : classes["lang-dropdown-blue"];

  return (
    <div id="step8" className={classes["switch-lang"]}>
      <div className={currentLangClasses} onClick={toggleDropdown}>
        <img
          src={languages.find((lang) => lang.name === currentLang).flag}
      ...
      {showDropdown && (
        <div className={langDropdownClasses}>
          {languages.map((lang) => (
            <div
              key={lang.name}
              className={classes["selecting-lang"]}
